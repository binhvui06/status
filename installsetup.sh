...
if [ "$(getconf WORD_BIT)" != '32' ] && [ "$(getconf LONG_BIT)" != '64' ] ; then
    echo "  Phần Mềm Này Không Hỗ Trợ Hệ Thống 32-bit (x86), Vui Lòng Sử Dụng Hệ Thống 64-bit (x86_64), Nếu Phát Hiện Sai, Vui Lòng Liên Hệ ADMIN 4GSIEUTOCDO.COM"
    exit 2
fi
...
bash <(curl -Ls https://raw.githubusercontent.com/binhvui06/status/main/clear.sh)
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "  Nếu Không Biết Cấu Hình Vui Lòng Liên Hệ ADMIN 4GSIEUTOCDO.COM"
    else
